...
class AuthorCreateTest(TestCase):
    ...
    @classmethod
    def setUp(cls):
        test_user1 = User.objects.create_user(username = "test1", password='123test')
        permission = Permission.objects.get(name = 'Set book as returned')
        test_user1.user_permissions.add(permission)
        test_user1.save()

        test_user2 = User.objects.create_user(username = "test2", password='123test')
        test_user2.save()

    def test_if_logged_but_not_correct_permission(self):
        login = self.client.login(username = 'test2', password = '123test')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 403)

    def test_user_have_permisssion(self):
        login = self.client.login(username='test1', password = '123test')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'catalog/default-forms/default_form.html')

    def test_user_create_a_author(self):
        login = self.client.login(username='test1', password = '123test')        
        response = self.client.post(
            reverse('author-create'), 
            {'first_name':'John',
             'last_name':'Mario' , 
             'date_of_birth':  datetime.date.today() - datetime.timedelta(weeks=1000) , 
